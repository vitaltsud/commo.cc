...
ssh "${SSH_OPTS[@]}" "$COMMO_SSH" "[ -s /root/.nvm/nvm.sh ] && . /root/.nvm/nvm.sh; cd $COMMO_REMOTE_PATH && git pull && (test -f package-lock.json && npm ci || npm install) && npm run build && (pm2 restart commo 2>/dev/null || pm2 start npm --name commo -- start 2>/dev/null || echo 'Start app manually')"
echo "Done."
