...
Сервер: **84.247.164.76** (Kontabo). Существующие системы не менять — только добавлять сервисы и конфиги. На этом же сервере отдельно работает **n8n** (https://ai.crmca.cc/); его конфигурация не должна пострадать.
